...
Ext.ux.TreeOutlineFormats = function() {
    ...
    var romanNumerals = [romanOnes, romanTens, romanHundreds];
    return {
        /**
        ...
        romanUC: function(idx) {
            return this.roman(idx).toUpperCase();
        },
...
Ext.ux.TreeOutliner = function(config) {
    var defConfig = {
        delimiter: ':',
        outlineStyle: [Ext.ux.TreeOutlineFormats.alphaUC, Ext.ux.TreeOutlineFormats.numeric, Ext.ux.TreeOutlineFormats.roman],
        ellipsis: false
    };
...
Ext.ux.TreeOutliner.prototype = {
    ...
    storeOrigText: function(n, override) {
        ...
        if(!n.origText || override) {
            n.origText = n.get('text');
        }
    },
    /**
     ...
     * @private
     */
    updateText: function(parent) {
        if(parent) {
            parent.cascadeBy(function(n) {
                this.storeOrigText(n);
                var depth = n.getDepth();
                var idx = n.parentNode ? n.parentNode.indexOf(n) : -1;
                if (idx != -1) {
                    var style = this.outlineStyle[(depth - 1) % this.outlineStyle.length];
                    var prefix = typeof style === 'function' ? style.call(Ext.ux.TreeOutlineFormats, idx, depth) : Ext.ux.TreeOutlineFormats.indexed(idx, style);
                    var txt = n.origText;
                    if(this.ellipsis) {
                        txt = Ext.util.Format.ellipsis(txt, this.ellipsis);
                    }
                    n.set('text', prefix + this.delimiter + txt);
                    n.commit();
                }
            }, this);
        }
